use crate::{
    common::{BinaryOp, FuncRef, Ident, Intrinsic, Literal, UnaryOp},
    high_level_ir as hir,
};

type Ty = Arc<hir::Type>;

type Expr<T = Ty> = hir::Expr<T>;

use std::{
    collections::{HashMap, HashSet},
    mem,
    sync::Arc,
};

use continuate_error::Result;

#[derive(Debug, Clone, PartialEq, Eq)]
#[expect(clippy::enum_variant_names)]
enum Exhaustive {
    Exhaustive,
    ExhaustiveVariants(HashSet<usize>),
    NonExhaustive,
}

impl Exhaustive {
    fn finalise(self, num_variants: Option<usize>) -> Exhaustive {
        match (self, num_variants) {
            (Exhaustive::Exhaustive, _) => Exhaustive::Exhaustive,
            (Exhaustive::ExhaustiveVariants(variants), Some(num_variants))
                if variants.len() == num_variants =>
            {
                Exhaustive::Exhaustive
            }
            _ => Exhaustive::NonExhaustive,
        }
    }

    fn intersect(self, other: Exhaustive) -> Exhaustive {
        match (self, other) {
            (Exhaustive::Exhaustive, Exhaustive::Exhaustive) => Exhaustive::Exhaustive,
            (Exhaustive::Exhaustive, Exhaustive::ExhaustiveVariants(variants))
            | (Exhaustive::ExhaustiveVariants(variants), Exhaustive::Exhaustive) => {
                Exhaustive::ExhaustiveVariants(variants)
            }
            (
                Exhaustive::ExhaustiveVariants(self_variants),
                Exhaustive::ExhaustiveVariants(other_variants),
            ) => Exhaustive::ExhaustiveVariants(
                self_variants
                    .intersection(&other_variants)
                    .copied()
                    .collect(),
            ),
            _ => Exhaustive::NonExhaustive,
        }
    }

    fn union(self, other: Exhaustive) -> Exhaustive {
        match (self, other) {
            (Exhaustive::Exhaustive, _) | (_, Exhaustive::Exhaustive) => Exhaustive::Exhaustive,
            (
                Exhaustive::ExhaustiveVariants(self_variants),
                Exhaustive::ExhaustiveVariants(other_variants),
            ) => Exhaustive::ExhaustiveVariants(
                self_variants.union(&other_variants).copied().collect(),
            ),
            (Exhaustive::ExhaustiveVariants(variants), Exhaustive::NonExhaustive)
            | (Exhaustive::NonExhaustive, Exhaustive::ExhaustiveVariants(variants)) => {
                Exhaustive::ExhaustiveVariants(variants)
            }
            (Exhaustive::NonExhaustive, Exhaustive::NonExhaustive) => Exhaustive::NonExhaustive,
        }
    }
}

struct TypeCk<'a> {
    program: &'a hir::Program<()>,
    typed_program: hir::Program<Ty>,
    environment: HashMap<Ident, Ty>,
    closures: Vec<(FuncRef, HashMap<Ident, Ty>)>,
}

impl<'a> TypeCk<'a> {
    fn exprs<'b, I: IntoIterator<Item = &'b hir::Expr<()>>>(
        &mut self,
        exprs: I,
    ) -> Result<Vec<hir::Typed<Expr>>> {
        exprs.into_iter().map(|expr| self.expr(expr)).collect()
    }

    fn expr_literal(&mut self, literal: Literal) -> hir::Typed<Expr> {
        let ty = match literal {
            Literal::Int(_) => self.typed_program.insert_type(hir::Type::Int),
            Literal::Float(_) => self.typed_program.insert_type(hir::Type::Float),
            Literal::String(_) => self.typed_program.insert_type(hir::Type::String),
        };
        hir::Typed::new(hir::Expr::Literal(literal), ty)
    }

    fn expr_ident(&self, expr: &hir::ExprIdent<()>) -> Result<hir::Typed<Expr>> {
        let ty = self
            .environment
            .get(&expr.ident)
            .cloned()
            .ok_or_else(|| format!("cannot find {:?}", expr.ident))?;
        Ok(hir::Typed::new(
            hir::Expr::Ident(hir::ExprIdent {
                ident: hir::Typed::new(expr.ident, Arc::clone(&ty)),
            }),
            ty,
        ))
    }

    fn expr_function(&self, func_ref: FuncRef) -> hir::Typed<Expr> {
        let ty = Arc::clone(&self.typed_program.signatures[&func_ref]);
        hir::Typed::new(hir::Expr::Function(func_ref), ty)
    }

    fn block(&mut self, exprs: &[Expr<()>]) -> Result<hir::Typed<Vec<Expr>>> {
        let Some((tail, block)) = exprs.split_last() else {
            let ty = self.typed_program.insert_type(hir::Type::Tuple(Vec::new()));
            return Ok(hir::Typed::new(vec![], Arc::clone(&ty)));
        };

        let block: Result<Vec<_>> = block
            .iter()
            .map(|expr| Ok(self.expr(expr)?.value))
            .collect();
        let mut block = block?;

        let tail = self.expr(tail)?;
        block.push(tail.value);
        Ok(hir::Typed::new(block, tail.ty))
    }

    fn expr_block(&mut self, expr: &hir::ExprBlock<()>) -> Result<hir::Typed<Expr>> {
        self.block(&expr.exprs).map(|x| {
            hir::Typed::new(
                Expr::Block(hir::Typed::new(
                    hir::ExprBlock { exprs: x.value },
                    Arc::clone(&x.ty),
                )),
                x.ty,
            )
        })
    }

    fn expr_tuple(&mut self, expr: &hir::ExprTuple<()>) -> Result<hir::Typed<Expr>> {
        let exprs = self.exprs(&expr.exprs)?;
        let ty = self.typed_program.insert_type(hir::Type::Tuple(
            exprs.iter().map(|x| Arc::clone(&x.ty)).collect(),
        ));
        Ok(hir::Typed::new(
            Expr::Tuple(hir::Typed::new(
                hir::ExprTuple {
                    exprs: exprs.into_iter().map(|x| x.value).collect(),
                },
                Arc::clone(&ty),
            )),
            ty,
        ))
    }

    fn expr_constructor(&mut self, expr: &hir::ExprConstructor<()>) -> Result<hir::Typed<Expr>> {
        let ty = Arc::clone(&expr.ty);
        let user_defined = ty
            .as_user_defined()
            .ok_or_else(|| format!("cannot construct {:?}", expr.ty))?;
        let ty_fields = match (
            &*self.program.user_defined_types[&user_defined],
            &expr.variant,
        ) {
            (hir::UserDefinedType::Product(ty_fields), None) => ty_fields.clone(),
            (hir::UserDefinedType::Sum(variants), Some(variant)) => variants
                .iter()
                .find(|(name, _)| name == variant)
                .map(|(_, variant)| variant.clone())
                .ok_or_else(|| format!("type {:?} has no variant '{variant}'", expr.ty))?,
            _ => unreachable!(),
        };

        let fields = match (&expr.fields, ty_fields) {
            (
                hir::ExprConstructorFields::Named(expr_fields),
                hir::UserDefinedTypeFields::Named(ty_fields),
            ) => {
                let mut fields = Vec::new();
                for (field, expr) in expr_fields {
                    if fields.iter().any(|(f, _)| f == field) {
                        Err(format!("field {field} specified twice"))?;
                    }

                    let (field, ty_field) = ty_fields
                        .iter()
                        .find(|(name, _)| name == field)
                        .ok_or_else(|| format!("type {ty:?} has no field '{field}"))?;
                    let ty_field = Arc::clone(ty_field);
                    let expr = self.expr(expr)?;
                    expr.ty.unify(&ty_field, &mut self.typed_program)?;
                    fields.push((field.clone(), expr.value));
                }
                for (field, _) in &ty_fields {
                    if !fields.iter().any(|(f, _)| f == field) {
                        Err(format!("missing field '{field}'"))?;
                    }
                }
                hir::ExprConstructorFields::Named(fields)
            }
            (
                hir::ExprConstructorFields::Anonymous(expr_fields),
                hir::UserDefinedTypeFields::Anonymous(ty_fields),
            ) => {
                let expr_fields = self.exprs(expr_fields)?;
                if expr_fields.len() != ty_fields.len() {
                    Err("incorrect number of fields")?;
                }
                for (expr, field_ty) in expr_fields.iter().zip(ty_fields) {
                    expr.ty.unify(&field_ty, &mut self.typed_program)?;
                }
                hir::ExprConstructorFields::Anonymous(
                    expr_fields.into_iter().map(|x| x.value).collect(),
                )
            }
            (hir::ExprConstructorFields::Unit, hir::UserDefinedTypeFields::Unit) => {
                hir::ExprConstructorFields::Unit
            }
            _ => Err("incompatible field styles")?,
        };

        let expr = hir::ExprConstructor {
            ty: Arc::clone(&ty),
            variant: expr.variant.clone(),
            fields,
        };

        Ok(hir::Typed::new(Expr::Constructor(expr), ty))
    }

    fn expr_array(&mut self, expr: &hir::ExprArray<()>) -> Result<hir::Typed<Expr>> {
        let exprs = self.exprs(&expr.exprs)?;
        let element_ty = exprs.iter().try_fold(
            self.typed_program.insert_type(hir::Type::Unknown),
            |acc, val| val.ty.unify(&acc, &mut self.typed_program),
        )?;
        let ty = self.typed_program.insert_type(hir::Type::Array(
            Arc::clone(&element_ty),
            exprs.len() as u64,
        ));
        let expr = Expr::Array(hir::Typed::new(
            hir::ExprArray {
                exprs: exprs.into_iter().map(|x| x.value).collect(),
            },
            Arc::clone(&ty),
        ));
        Ok(hir::Typed::new(expr, ty))
    }

    fn expr_get(&mut self, expr: &hir::ExprGet<()>) -> Result<hir::Typed<Expr>> {
        let (object, object_ty) = self.expr(&expr.object)?.into_pair();
        let hir::Type::UserDefined(ty) = *object_ty else {
            Err(format!("cannot take field of {object_ty:?}"))?
        };
        let hir::UserDefinedType::Product(fields) = &*self.program.user_defined_types[&ty] else {
            Err(format!("cannot take field of {object_ty:?}"))?
        };

        let field_ty = fields
            .get(&expr.field)
            .cloned()
            .ok_or_else(|| format!("{object_ty:?} has no field '{}'", &expr.field))?;
        let expr = Expr::Get(hir::ExprGet {
            object: hir::Typed::new(Box::new(object), object_ty),
            field: expr.field.clone(),
        });

        Ok(hir::Typed::new(expr, field_ty))
    }

    fn expr_set(&mut self, expr: &hir::ExprSet<()>) -> Result<hir::Typed<Expr>> {
        let (object, object_ty) = self.expr(&expr.object)?.into_pair();

        let hir::Type::UserDefined(ty) = &*object_ty else {
            Err(format!("cannot take field of {object_ty:?}"))?
        };
        let hir::UserDefinedType::Product(fields) = &*self.program.user_defined_types[ty] else {
            Err(format!("cannot take field of {ty:?}"))?
        };

        let field_ty = fields
            .get(&expr.field)
            .cloned()
            .ok_or_else(|| format!("{object_ty:?} has no field '{}'", expr.field))?;

        let (value, value_ty) = self.expr(&expr.value)?.into_pair();

        value_ty.unify(&field_ty, &mut self.typed_program)?;

        let expr = Expr::Set(hir::ExprSet {
            object: hir::Typed::new(Box::new(object), object_ty),
            field: expr.field.clone(),
            value: Box::new(value),
        });
        Ok(hir::Typed::new(expr, field_ty))
    }

    // fn expr_call(&mut self, expr: &hir::ExprCall<()>) -> Result<hir::Typed<Expr>> {
    //     let callee = self.expr(&expr.callee)?;
    //     let hir::Type::Function(hir::FunctionTy {
    //         positional_params,
    //         named_params,
    //     }) = &*callee.ty
    //     else {
    //         Err(format!("{:?} is not a function", callee.ty))?
    //     };

    //     if !named_params.is_empty() {
    //         Err("cannot call a function with remaining named parameters")?;
    //     }

    //     if expr.positional.len() != positional_params.len() {
    //         Err(format!(
    //             "incorrect number of arguments (expected {}, got {})",
    //             positional_params.len(),
    //             expr.positional.len()
    //         ))?;
    //     }

    //     let args: Result<Vec<_>> = positional_params
    //         .iter()
    //         .zip(&expr.positional)
    //         .map(|(param, arg)| {
    //             let (arg, ty) = self.expr(arg)?.into_pair();
    //             ty.unify(param, &mut self.typed_program)?;
    //             Ok(arg)
    //         })
    //         .collect();

    //     let expr = hir::ExprCall {
    //         callee: callee.boxed(),
    //         positional: args?,
    //         named: vec![],
    //     };

    //     Ok(hir::Typed::new(
    //         Expr::Call(expr),
    //         self.typed_program.insert_type(hir::Type::None),
    //     ))
    // }

    fn expr_call(&mut self, expr: &hir::ExprCall<()>) -> Result<hir::Typed<Expr>> {
        let callee = self.expr(&expr.callee)?;
        let hir::Type::Function(hir::FunctionTy {
            positional_params,
            named_params,
        }) = &*callee.ty
        else {
            Err("{ty:?} is not a function")?
        };

        if expr.positional.len() > positional_params.len() {
            Err(format!(
                "too many positional parameters (expected up to {}, got {})",
                positional_params.len(),
                expr.positional.len()
            ))?;
        }

        let mut positional_args = Vec::with_capacity(expr.positional.len());
        for (i, arg) in expr.positional.iter().enumerate() {
            let arg = self.expr(arg)?;
            arg.ty
                .unify(&positional_params[i], &mut self.typed_program)?;
            positional_args.push(arg.value);
        }
        let positional_params = positional_params[expr.positional.len()..].to_vec();

        let mut named_params = named_params.clone();
        let mut named_args = Vec::with_capacity(expr.named.len());
        for (ident, cont) in &expr.named {
            let cont = self.expr(cont)?;
            let expected = named_params
                .remove(ident)
                .ok_or_else(|| format!("no such continuation {ident:?}"))?;
            cont.ty.unify(&expected, &mut self.typed_program)?;
            named_args.push((*ident, cont.value));
        }

        if positional_params.is_empty() && named_params.is_empty() {
            Ok(hir::Typed::new(
                hir::Expr::Call(hir::ExprCall {
                    callee: callee.boxed(),
                    positional: positional_args,
                    named: named_args,
                }),
                self.typed_program.insert_type(hir::Type::None),
            ))
        } else {
            let ty = hir::Type::function(positional_params, named_params);
            let ty = self.typed_program.insert_type(ty);
            let expr = hir::Typed::new(
                hir::ExprApplication {
                    callee: callee.boxed(),
                    positional: positional_args,
                    named: named_args,
                },
                Arc::clone(&ty),
            );
            Ok(hir::Typed::new(Expr::Application(expr), ty))
        }
    }

    fn expr_unary(&mut self, expr: &hir::ExprUnary<()>) -> Result<hir::Typed<Expr>> {
        let right = self.expr(&expr.right)?;
        let ty = match (expr.op, &*right.ty) {
            (UnaryOp::Neg, hir::Type::Int | hir::Type::Float) | (UnaryOp::Not, hir::Type::Bool) => {
                Ok(Arc::clone(&right.ty))
            }
            _ => Err(format!("invalid use of {:?}", expr.op)),
        }?;
        let expr = hir::ExprUnary {
            op: expr.op,
            right: right.boxed(),
        };
        Ok(hir::Typed::new(Expr::Unary(expr), ty))
    }

    fn expr_binary(&mut self, expr: &hir::ExprBinary<()>) -> Result<hir::Typed<Expr>> {
        let left = self.expr(&expr.left)?;

        let right = self.expr(&expr.right)?;

        let ty = match (&*left.ty, expr.op, &*right.ty) {
            (
                l @ (hir::Type::Int | hir::Type::Float | hir::Type::String),
                BinaryOp::Add,
                r @ (hir::Type::Int | hir::Type::Float | hir::Type::String),
            )
            | (
                l @ (hir::Type::Int | hir::Type::Float),
                BinaryOp::Sub | BinaryOp::Mul | BinaryOp::Div | BinaryOp::Rem,
                r @ (hir::Type::Int | hir::Type::Float),
            ) if l == r => Ok(Arc::clone(&left.ty)),
            (
                l @ (hir::Type::Int | hir::Type::Float | hir::Type::String),
                BinaryOp::Lt | BinaryOp::Le | BinaryOp::Gt | BinaryOp::Ge,
                r @ (hir::Type::Int | hir::Type::Float | hir::Type::String),
            ) if l == r => Ok(self.typed_program.insert_type(hir::Type::Bool)),
            (l, BinaryOp::Eq | BinaryOp::Ne, r) if l == r => {
                Ok(self.typed_program.insert_type(hir::Type::Bool))
            }
            _ => Err(format!(
                "cannot apply {:?} to {:?} and {:?}",
                expr.op, left.ty, right.ty
            )),
        }?;

        let expr = hir::ExprBinary {
            left: left.boxed(),
            op: expr.op,
            right: right.boxed(),
        };
        Ok(hir::Typed::new(Expr::Binary(expr), ty))
    }

    fn expr_declare(&mut self, expr: &hir::ExprDeclare<()>) -> Result<hir::Typed<Expr>> {
        let typed_expr = self.expr(&expr.expr)?;
        let ty = typed_expr.ty.unify(&expr.ty, &mut self.typed_program)?;
        self.environment.insert(expr.ident, Arc::clone(&ty));
        let expr = hir::ExprDeclare {
            ident: expr.ident,
            ty: Arc::clone(&ty),
            expr: Box::new(typed_expr.value),
        };
        Ok(hir::Typed::new(Expr::Declare(expr), ty))
    }

    fn expr_assign(&mut self, expr: &hir::ExprAssign<()>) -> Result<hir::Typed<Expr>> {
        let object = self.expr(&expr.expr)?;
        let expected = self.environment.get(&expr.ident).unwrap();
        let ty = object.ty.unify(expected, &mut self.typed_program)?;
        let expr = hir::ExprAssign {
            ident: expr.ident,
            expr: Box::new(object.value),
        };
        Ok(hir::Typed::new(Expr::Assign(expr), ty))
    }

    fn pattern(&mut self, expr_ty: Ty, pattern: &hir::Pattern) -> Result<Exhaustive> {
        match *pattern {
            hir::Pattern::Wildcard => Ok(Exhaustive::Exhaustive),
            hir::Pattern::Ident(ident) => {
                self.environment.insert(ident, expr_ty);
                Ok(Exhaustive::Exhaustive)
            }
            hir::Pattern::Destructure {
                ref ty,
                ref variant,
                ref fields,
            } => {
                let ty = expr_ty.unify(ty, &mut self.typed_program)?;
                let user_defined =
                    Arc::clone(&self.program.user_defined_types[&ty.as_user_defined().unwrap()]);
                let field_tys = match (&*user_defined, variant) {
                    (hir::UserDefinedType::Product(fields), None) => fields,
                    (hir::UserDefinedType::Sum(variants), Some(variant)) => variants
                        .iter()
                        .find(|(name, _)| name == variant)
                        .map(|(_, variant)| variant)
                        .ok_or_else(|| format!("{ty:?} has no variant '{variant}'"))?,
                    (hir::UserDefinedType::Product(_), Some(variant)) => {
                        Err(format!("{ty:?} has no variant '{variant}'"))?
                    }
                    (hir::UserDefinedType::Sum(_), None) => {
                        Err(format!("{ty:?} does not have fields"))?
                    }
                };

                let exhaustive = match (fields, field_tys) {
                    (
                        hir::DestructureFields::Named(fields),
                        hir::UserDefinedTypeFields::Named(field_tys),
                    ) => {
                        let mut used_fields = HashSet::new();
                        let mut exhaustive = Exhaustive::Exhaustive;
                        for (field, pat) in fields {
                            let (_, field_ty) = field_tys
                                .iter()
                                .find(|(name, _)| name == field)
                                .ok_or_else(|| format!("type {ty:?} has no field '{field}'"))?;
                            exhaustive = self
                                .pattern(Arc::clone(field_ty), pat)?
                                .finalise(None)
                                .intersect(exhaustive);
                            used_fields.insert(field);
                        }
                        for (field, _) in field_tys {
                            if !used_fields.contains(field) {
                                Err(format!("missing field '{field}'"))?;
                            }
                        }
                        exhaustive
                    }
                    (
                        hir::DestructureFields::Anonymous(fields),
                        hir::UserDefinedTypeFields::Anonymous(field_tys),
                    ) => fields.iter().zip(field_tys).try_fold(
                        Exhaustive::Exhaustive,
                        |acc, (pat, field_ty)| {
                            self.pattern(Arc::clone(field_ty), pat)
                                .map(|e| e.finalise(None).intersect(acc))
                        },
                    )?,
                    (hir::DestructureFields::Unit, hir::UserDefinedTypeFields::Unit) => {
                        Exhaustive::Exhaustive
                    }
                    _ => return Err("mismatched field types".into()),
                };

                match (variant, exhaustive) {
                    (Some(variant), Exhaustive::Exhaustive) => {
                        let user_defined =
                            &self.program.user_defined_types[&ty.as_user_defined().unwrap()];
                        let variants = user_defined.as_sum().unwrap();
                        let (variant, _) = variants
                            .iter()
                            .enumerate()
                            .find(|(_, (name, _))| name == variant)
                            .unwrap();
                        let mut found_variants = HashSet::with_capacity(1);
                        found_variants.insert(variant);
                        Ok(Exhaustive::ExhaustiveVariants(found_variants))
                    }
                    (None, Exhaustive::Exhaustive) => Ok(Exhaustive::Exhaustive),
                    _ => Ok(Exhaustive::NonExhaustive),
                }
            }
        }
    }

    fn expr_match(&mut self, expr: &hir::ExprMatch<()>) -> Result<hir::Typed<Expr>> {
        let scrutinee = self.expr(&expr.scrutinee)?;
        let (exhaustive, output_ty, arms) = expr.arms.iter().try_fold(
            (
                Exhaustive::Exhaustive,
                self.typed_program.insert_type(hir::Type::Unknown),
                vec![],
            ),
            |(exhaustive, output_ty, mut arms), (pat, expr)| {
                let expr = self.expr(expr)?;
                let output_ty = expr.ty.unify(&output_ty, &mut self.typed_program)?;
                let arm_exhaustive = self.pattern(expr.ty, pat)?;
                arms.push((pat.clone(), expr.value));
                Result::<_>::Ok((exhaustive.union(arm_exhaustive), output_ty, arms))
            },
        )?;
        if exhaustive.finalise(None) != Exhaustive::Exhaustive {
            Err("non-exhaustive match")?;
        }
        let expr = hir::ExprMatch {
            scrutinee: scrutinee.boxed(),
            arms,
        };
        Ok(hir::Typed::new(Expr::Match(expr), output_ty))
    }

    fn expr_closure(&mut self, expr: &hir::ExprClosure) -> hir::Typed<Expr> {
        let actual_func = self.program.functions.get(&expr.func).unwrap();
        let captures: HashMap<_, _> = actual_func
            .captures
            .iter()
            .map(|&ident| (ident, Arc::clone(&self.environment[&ident])))
            .collect();
        self.closures.push((expr.func, captures.clone()));
        let ty = Arc::clone(&self.program.signatures[&expr.func]);
        let expr = hir::ExprClosure {
            func: expr.func,
            captures,
        };
        hir::Typed::new(Expr::Closure(expr), ty)
    }

    fn expr_intrinsic(&mut self, expr: &hir::ExprIntrinsic<()>) -> Result<hir::Typed<Expr>> {
        let values = self.exprs(&expr.values)?;
        let ty = match expr.intrinsic {
            Intrinsic::Discriminant => self.typed_program.insert_type(hir::Type::Int),
            Intrinsic::Terminate | Intrinsic::Unreachable => {
                self.typed_program.insert_type(hir::Type::None)
            }
        };
        let expr = hir::ExprIntrinsic {
            intrinsic: expr.intrinsic,
            values,
        };
        Ok(hir::Typed::new(Expr::Intrinsic(expr), ty))
    }

    fn expr(&mut self, expr: &hir::Expr<()>) -> Result<hir::Typed<Expr>> {
        match expr {
            hir::Expr::Literal(literal) => Ok(self.expr_literal(literal.clone())),
            hir::Expr::Ident(ident) => self.expr_ident(ident),
            hir::Expr::Function(func_ref) => Ok(self.expr_function(*func_ref)),
            hir::Expr::Block(expr) => self.expr_block(expr),
            hir::Expr::Tuple(expr) => self.expr_tuple(expr),
            hir::Expr::Constructor(expr) => self.expr_constructor(expr),
            hir::Expr::Array(expr) => self.expr_array(expr),
            hir::Expr::Get(expr) => self.expr_get(expr),
            hir::Expr::Set(expr) => self.expr_set(expr),
            hir::Expr::Call(expr) => self.expr_call(expr),
            hir::Expr::Application(_) => {
                unreachable!("application expressions are only created during type checking")
            }
            hir::Expr::Unary(expr) => self.expr_unary(expr),
            hir::Expr::Binary(expr) => self.expr_binary(expr),
            hir::Expr::Declare(expr) => self.expr_declare(expr),
            hir::Expr::Assign(expr) => self.expr_assign(expr),
            hir::Expr::Match(expr) => self.expr_match(expr),
            hir::Expr::Closure(expr) => Ok(self.expr_closure(expr)),
            hir::Expr::Intrinsic(expr) => self.expr_intrinsic(expr),
        }
    }

    fn function(
        &mut self,
        function: &hir::Function<()>,
        captures: HashMap<Ident, Ty>,
    ) -> Result<hir::Function<Ty>> {
        self.environment = captures;

        for (&param, ty) in function
            .positional
            .iter()
            .map(|(param, ty)| (param, ty))
            .chain(&function.named)
        {
            self.environment.insert(param, Arc::clone(ty));
        }

        let ty_none = self.typed_program.insert_type(hir::Type::None);
        let (body, body_ty) = self.block(&function.body)?.into_pair();
        body_ty.unify(&ty_none, &mut self.typed_program)?;

        Ok(hir::Function::clone_metadata(function, body))
    }

    fn typeck(mut self) -> Result<hir::Program<Ty>> {
        for (&func_ref, function) in &self.program.functions {
            let positional = function
                .positional
                .iter()
                .map(|(_, ty)| Arc::clone(ty))
                .collect();
            let named = function
                .named
                .iter()
                .map(|(&name, ty)| (name, Arc::clone(ty)))
                .collect();
            let ty = self
                .typed_program
                .insert_type(hir::Type::function(positional, named));
            self.typed_program.signatures.insert(func_ref, ty);
        }

        for (&func_ref, function) in &self.program.functions {
            if !function.captures.is_empty() {
                continue;
            }

            let function = self.function(function, HashMap::new())?;

            self.typed_program.functions.insert(func_ref, function);
        }

        for (func_ref, captures) in mem::take(&mut self.closures) {
            let function = self.program.functions.get(&func_ref).unwrap();
            let function = self.function(function, captures)?;
            self.typed_program.functions.insert(func_ref, function);
        }

        Ok(self.typed_program)
    }

    fn new(program: &'a hir::Program<()>) -> TypeCk<'a> {
        TypeCk {
            program,
            typed_program: hir::Program::clone_metadata(program),
            environment: HashMap::new(),
            closures: Vec::new(),
        }
    }
}

/// # Errors
///
/// Returns an error if `program` contains a type error.
#[inline]
pub fn typeck(program: &hir::Program<()>) -> Result<hir::Program<Ty>> {
    TypeCk::new(program).typeck()
}
